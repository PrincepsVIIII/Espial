...
| Malicious HTTP/TLS peer | Whole-stage timeouts, header/body caps, minimum TLS, Go-native hostname/chain verification with approved roots and injected time, bounded evidence recovered from verification errors, no disabled certificate checks, no raw bodies/chains | Operations owns trust-root and monitored-endpoint inventory |
